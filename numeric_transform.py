...
def ILT_hyperbolic(func, t, N, alpha=0.0):
    ...
    # integration contour
    p = np.transpose(((np.arange(N) + 0.5)*h)[np.newaxis])
...
def IFT(func, profile, x, y, Nq, Np):
    ...
    r_limit = 3
    (r, dr) = np.linspace(-r_limit, r_limit, Nq, retstep=True)
...
def ILT_IFT_hyperbolic(func, profile, x, y, t, Ns, Nq, Np, alpha=0.0):
    ...
    """
    def diff_wrapper(s, q, phi):
        d = np.exp(1j*q*(x*np.cos(phi)+y*np.sin(phi)))
        return func(s, q, phi)*d*profile(q, phi)
    ...
    for i in range(Ns):
        F = diff_wrapper(s_int[i], q_int[:, None], phi_int[None, :])
        F = np.sum(F, axis=1)
        F = np.sum(F*qdq_int, axis=0)
